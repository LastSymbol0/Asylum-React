...
export default {
    ...
    return {
      ...
      windowBottom: {
        ...
        fromBetweenMiddleBottom:
          ...
          (domElementCoords.bottom - domElementCoords.height / 4),
        fromBottom:
          window.scrollY + window.innerHeight - domElementCoords.bottom,
      },
    }
  },
  ...
  isElementVisibleX(domElement) {
    ...
    return (
      domElementCoords.right >= 0 && domElementCoords.left <= window.innerWidth
    )
  },
  ...
  isElementHovered(domElement, additionalRadius = 0) {
    ...
    const mouseCoords = this.getMouseDocumentCoords()
    return (
      domElementCoords.top - additionalRadius < mouseCoords.y &&
      domElementCoords.bottom + additionalRadius > mouseCoords.y &&
      domElementCoords.left - additionalRadius < mouseCoords.x &&
      domElementCoords.right + additionalRadius > mouseCoords.x
    )
  },
  ...
  setToRender({
    ...
    if (!handler) {
      console.error(
        `Renderer: Handler for render is required. Handler label "${newLabel}"`
      )
      return
    }
    if (typeof handler !== 'function') {
      console.error(
        `Renderer: Invalid type of handler, required Function. Handler label "${newLabel}"`
      )
      return
    }
    if (breakpoint && typeof breakpoint !== 'number') {
      console.error(
        `Renderer: Invalid type of breakpoint, required Number. Handler label "${newLabel}"`
      )
      return
    }
    if (onBreakpoint && typeof onBreakpoint !== 'function') {
      console.error(
        `Renderer: Invalid type of onBreakpoint, required Function. Handler label "${newLabel}"`
      )
      return
    }
