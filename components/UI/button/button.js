...
const ButtonOutlineGradient = ({ className, type, active, children, link, ...props }) => {
    type = type ? type : types['small']  
    active = active ? active : isActive['false']  
    if (link) {
        return (
            <div { ...props } className={cn(styles.button, styles.buttonOutline, className, types[type], isActive[active])}>
                <a href={link || '#'} target="_blank" rel="noreferrer" type='button'>{ children }</a>
            </div>
        )
    }
    return (
